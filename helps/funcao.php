    ...
    if (!empty($sessao)) {
        ...
        if (!empty($id)) {
            return array(
                ...
                'produtos_qtd_total' => $somaProdutosQtd,
                'produtos_fav' => $produtosFav // Adiciona o array de produtos ao retorno
...
{ //cookies, usuário não logado
    ...
    return array(
        ...
        "qtd_cart" => $qtd_cart,
        'produtos_cart' => $produtosCart,
        'produtos_qtd_total' => $produtos_qtd_total,
        'produtos_fav' => $produtosFav
    );
}
...
function sendEmail($mail, $email_destinatario, $assunto, $attbody, $html) //enviar email
{
    ...
    $query = "SELECT 
                (SELECT cl_valor FROM tb_parametros WHERE cl_id = 73) AS host_email,
                (SELECT cl_valor FROM tb_parametros WHERE cl_id = 64) AS nome_ecommerce,
                (SELECT cl_empresa FROM tb_empresa WHERE cl_id = 1) AS nome_site,
                (SELECT cl_valor FROM tb_parametros WHERE cl_id = 74) AS email_remetente,
                (SELECT cl_valor FROM tb_parametros WHERE cl_id = 62) AS senha_email,
                (SELECT cl_valor FROM tb_parametros WHERE cl_id = 77) AS porta_email,
                (SELECT cl_nome_fantasia FROM tb_empresa WHERE cl_id = 1) AS nome_fantasia,
                (SELECT cl_valor FROM tb_parametros WHERE cl_id = 44) AS whatsapp,
                (SELECT cl_valor FROM tb_parametros WHERE cl_id = 43) AS instagram,
                (SELECT cl_valor FROM tb_parametros WHERE cl_id = 80) AS facebook,
                (SELECT cl_valor FROM tb_parametros WHERE cl_id = 81) AS telefone
              FROM DUAL";

    $result = mysqli_query($conecta, $query);
    $params = mysqli_fetch_assoc($result);

    // Armazena os parâmetros em variáveis individuais
    $host_email = $params['host_email'];
    $nome_ecommerce = $params['nome_ecommerce'];
    $nome_site = utf8_encode($params['nome_site']);
    $email_remetente = $params['email_remetente'];
    $senha_email = $params['senha_email'];
    $porta_email = $params['porta_email'];
    $nome_fantasia = utf8_encode($params['nome_fantasia']);
    ...
    try {
        ...
        $html = "
        <div style='max-width: 700px; border:1px solid black; padding:5px; margin-left: auto; margin-right: auto; font-family: Arial, sans-serif;'>
            ...
            <div style='text-align: center; margin-top: 20px;'>
                <p>Se você tiver alguma dúvida, responda a esta mensagem ou entre em contato conosco pelo e‑mail 
                    <a href='mailto:$email_remetente' style='color: #007bff; text-decoration: none;'>$email_remetente</a>.
                </p>
            </div>
            <hr style='border: 0; border-top: 1px solid #eaeaea; margin: 20px 0;'>
            <div style='text-align: center;'>
                <p style='font-size: 0.8em; color: #555;'>Atenciosamente, $nome_fantasia</p>
                <a href='https://api.whatsapp.com/send?phone=$whatsapp' style='margin-right: 10px;'>
                    <img src='https://i.pinimg.com/564x/26/88/29/268829190281a967d829180a7e0db375.jpg' alt='Whatsapp' style='width: 30px;'>
                </a>
                <a href='$instagram' style='margin-right: 10px;'>
                    <img src='https://toppng.com/uploads/preview/ew-instagram-logo-transparent-related-keywords-logo-instagram-vector-2017-115629178687gobkrzwak.png' alt='Instagram' style='width: 30px;'>
                </a>
                <a href='$facebook'>
                    <img src='https://i1.wp.com/www.multarte.com.br/wp-content/uploads/2019/03/logo-facebook-png.png?fit=696%2C696&ssl=1' alt='Facebook' style='width: 30px;'>
                </a>
            </div>
        </div>";
...
function sendEmailConfirm($mail, $assunto, $attbody, $html)
{
    ...
    $query = "SELECT 
     ...
     (SELECT cl_valor FROM tb_parametros WHERE cl_id = 80) AS facebook,
     (SELECT cl_valor FROM tb_parametros WHERE cl_id = 81) AS telefone,
     (SELECT cl_valor FROM tb_parametros WHERE cl_id = 98) AS email_destinatarios_param
   FROM DUAL";
    ...
    $email_destinatarios_param = $params['email_destinatarios_param']; // Endereço de email que irão receber email de confirmação - loja (separado por espaço)
    ...
    try {
        ...
        $html = "<div style='max-width: 700px;border:1px solid black; padding:10px; margin-left: auto; margin-right: auto; font-family: Arial, sans-serif;'>
            <div style='text-align: center; padding: 20px 0;'>
                <img src='$url_init_img/$nome_site/img/ecommerce/logo/logo.png' alt='Logo' width='120'>
            </div>
        <h3>Novo Pedido!</h3>
        ...
        foreach ($email_destinatarios_array as $email_dest) {
            // Adiciona o destinatário ao e-mail
            $mail->addAddress(trim($email_dest));
        }
...
function sendEmailDefaultFromAdm($mail, $assunto, $attbody, $html)
{
    global $conecta;
    global $url_init_img;
    // Realiza uma única consulta para obter todos os parâmetros necessários
   ...
   FROM DUAL";
    ...
    try {
        ...
        // Adiciona os destinatários
        $html = "
        <div style='max-width: 700px;border:1px solid black; padding:10px; margin-left: auto; margin-right: auto; font-family: Arial, sans-serif;'>
                <div style='text-align: center; padding: 20px 0;'>
                    <img src='$url_init_img/$nome_site/img/ecommerce/logo/logo.png' alt='Logo' width='120'>
                </div>" . $html .
            "</div>";
...
function buscar_cep($cep)
{
    ...
    $tamanhoDaString = strlen($cep);

    if (empty($cep)) {
        $retornar["data"] = array("status" => false, "message" => "Informe o cep");
    } elseif ($tamanhoDaString == 8) {
...
function simularFreteKangu(
    ...
    $qtd_postagem =  (consulta_tabela('tb_parametros', 'cl_id', '135', 'cl_valor')); //quantidade de dias para a postagem do produto a transpotadora
    ...
    if ($httpCode == "200") { // Operação bem-sucedida
        $cepDestino = $responseCep['cep'];
        ...
        // Inicializar variáveis de prazo com valores altos para comparação
        $menorPrazoEnt = PHP_INT_MAX;
        $menorPrazoEntMin = PHP_INT_MAX;
        $transp_nome_frete_gratis = '';
        $dtPrevEnt = '';
        // Iterar sobre os valores de resposta e coletar o menor prazo
        foreach ($response as $resp) {
            if (isset($resp['prazoEnt']) && $resp['prazoEnt'] < $menorPrazoEnt) {
                $menorPrazoEnt = $resp['prazoEnt'];
                $transp_nome_frete_gratis = $resp['transp_nome'];
                $dtPrevEnt = $resp['dtPrevEnt'];
                $dtPrevEntMin = $resp['dtPrevEntMin'];
            }
            if (isset($resp['prazoEntMin']) && $resp['prazoEntMin'] < $menorPrazoEntMin) {
                $menorPrazoEntMin = $resp['prazoEntMin'];
            }
        }

        if ($pFreteGratisEstado == true) { // Frete grátis dentro do estado
            $response[] = array(
                ...
                "idTranspResp" => 0,
                "nf_obrig" => "N",
                "prazoEnt" => $menorPrazoEnt, // Substitui pelo menor prazo encontrado
                "prazoEntMin" => $menorPrazoEntMin, // Substitui pelo menor prazo mínimo encontrado
                ...
                "transp_nome" => "Frete Grátis - $transp_nome_frete_gratis",
                "url_logo" => "https://portal.kangu.com.br/ged/documento/download/file/3962/Logo_Correios.png",
                "vlrFrete" => 0
            );
        } elseif ($pFreteGratisForaEstado == true) { // Frete grátis fora do estado
            $response[] = array(
                "cnpjTransp" => "99999999000000",
                "cnpjTranspResp" => "99999999000000",
                "descricao" => "Grátis",
        ...
        // Após todas as condições if
        foreach ($response as &$resp) {
            // Somar qtd_postagem ao prazoEnt e prazoEntMin, se existirem
            if (isset($resp['prazoEnt'])) {
                $resp['prazoEnt'] += $qtd_postagem;
            }
            if (isset($resp['prazoEntMin'])) {
                $resp['prazoEntMin'] += $qtd_postagem;
            }

            // Somar qtd_postagem às datas dtPrevEnt e dtPrevEntMin
            if (isset($resp['dtPrevEnt'])) {
                $dt = new DateTime($resp['dtPrevEnt']);
                $dt->modify("+$qtd_postagem days");
                $resp['dtPrevEnt'] = $dt->format('Y-m-d H:i:s');
            }

            if (isset($resp['dtPrevEntMin'])) {
                $dtMin = new DateTime($resp['dtPrevEntMin']);
                $dtMin->modify("+$qtd_postagem days");
                $resp['dtPrevEntMin'] = $dtMin->format('Y-m-d H:i:s');
            }
        }


        // Função para ordenar pelo campo 'vlrFrete'
        usort($response, function ($a, $b) {
            if ($a['vlrFrete'] == $b['vlrFrete']) {
                return 0;
            }
            return ($a['vlrFrete'] < $b['vlrFrete']) ? -1 : 1;
        });
...
function calcularValorCarrinho($payment, $simulacaoFrete, $selectedIdSimulacao, $codigo_nf, $cupom)
{
    ...
    if ($dados_usuario) { //subtotal
        ...
        if ($qtd_cart > 0) {
            ...
            if (($simulacaoFrete) != '') { //simulação de frete, valor do frete escolhido
                foreach ($simulacaoFrete as $simulacao) {
                    if ($simulacao['idSimulacao'] == $selectedIdSimulacao) {
                        ...
                        $dtPrevEnt = $simulacao["dtPrevEnt"];
                        $dtPrevEntMin = $simulacao["dtPrevEntMin"];
            ...
            $operadorValor = $validaCupom['operadorValor'];

            if ($valorCupom != 0) {
                if ($operadorValor == "percent") { //percent
...
function validaCupom($dados)
{
    global $conecta;
    global $data_lancamento;
    $total_produtos = $dados['valor_produto'];
    $cupom = $dados['cupom'];
    $retornar = array('msgCupom' => "", 'valorCupom' => 0, 'operadorValor' => "percent");
    ...
    if (!empty($cupom)) {
        ...
        if ($consulta) {
            ...
            if ($qtd_registro == 0) { //validar se existe o cupom
                ...
                if ($condicao_cadastrado == 1 and auth('') == false) { //requer cadastro do usuário para utilização do cupom
                    $retornar['msgCupom'] = "Cadastre-se para utilizar este cupom.";
                    return $retornar;
                } else {
                    // Verificação de data e status
                    if ($data_validade_inicial > $data_lancamento) { //validação da data inicial
                        $retornar['msgCupom'] = "Este cupom ainda não é válido. Ele será válido a partir de " . formatDateB($data_validade_inicial) . ".";
                    }

                    if ((($data_validade_final != '0000-00-00' or empty($data_validade_final)) and $data_validade_final < $data_lancamento) || $status == "0") { //validação da data final e o status
                    ...
                    WHERE cl_cupom = '$cupom' and cl_status_compra= 'CONCLUIDO'", 'total');
                        if ($limite_utilizado <= $validar_limite_cupom) {
                            $retornar['msgCupom']  = "Ops, Limite do cupom excedido";
                        }
                    }
                    if ($valor_minimo > 0) { // Validação do valor mínimo do cupom

                        if ($valor_minimo >= $total_produtos) {
                            $retornar['msgCupom'] = "Para aplicar este cupom, o valor mínimo de produtos no carrinho deve ser de " . real_format($valor_minimo);
                        }
                    }
                }
                if (auth('') != false) { //usuario está logado
                    ...
                    if ($limite_cliente == 1) { //validação para uso de 1 cupom por cliente
                        $valida_limite_compra = consulta_tabela_query($conecta, "SELECT count(*) as total FROM tb_pedido_loja 
                        WHERE ((cl_cpf_cnpj = '$cpf_cnpj' or cl_email = '$email') and cl_cupom='$cupom' and cl_status_compra= 'CONCLUIDO' and cl_status_pagamento ='approved')", 'total');

                        if ($valida_limite_compra > 0) {
                            $retornar['msgCupom'] = "Você atingiu o limite de 1 uso deste cupom por cliente.";
                        }
                    }
                ...
                if (empty($retornar['msgCupom'])) { //cupom  válido, disponibilizar o valor
                    ...
                    $retornar['valorCupom'] = $valor;
                    $retornar['operadorValor'] = $operador;
                }
            }
        }
    }
...
function inserirProdutoBd($codigo_nf, $nome_do_arquivo)
{
    ...
    if ($dados_usuario) {
        ...
        if ($qtd_cart > 0) {
         ...
         `cl_descricao`, `cl_referencia`, `cl_quantidade`, `cl_valor`) VALUES ('$codigo_nf', '$data', '$id', '$titulo $span_variacao', '$referencia', '$quantidade', '$total') ";
...
function gerarPagamentoMercadoPago($accesstoken, $id)
{
    ...
    $items = [];
    $remaining_discount = $valor_desconto;
    ...
    while ($linha = mysqli_fetch_assoc($consulta)) {
        ...
        // Aplicar desconto aos produtos até que o desconto seja totalmente utilizado
        if ($remaining_discount > 0) {
            $total_item_value = $quantidade * $valor;

            if ($total_item_value > $remaining_discount) {
                $valor = $valor - ($remaining_discount / $quantidade);
                $remaining_discount = 0; // Todo o desconto foi aplicado
            } else {
                $remaining_discount -= $total_item_value;
                $valor = 0; // Todo o valor do item é coberto pelo desconto
            }
        }
