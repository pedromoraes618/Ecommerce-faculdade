...
if (isset($_POST['form'])) {
    ...
    if ($acao == "create") { //crição do pedido
        ...
        $bairro_usuario = $bairro; //bairro definido pelo usuário
        $endereco_usuario = $logradouro; //endereço definido pelo usuário
        if (empty($nome)) {
        ...
        if (empty($cep)) {
            ...
            if ($simular_frete['data']['status'] == false) {
                ...
                /*verica se api de frete não encontrou o bairro ou endereço, se não será atribuido o que usyário informar */
                $bairro = empty($bairro) ? $bairro_usuario : $bairro;
                $endereco = empty($endereco) ? $endereco_usuario : $endereco;
                
                /*valiação do endereço */
                if (empty($numero)) {
                    $retornar["errors"]["numero"] = required("o número");
                }
                if (empty($bairro)) {
                    $retornar["errors"]["bairro"] = required("o bairro");
                }
                if (empty($endereco)) {
                    $retornar["errors"]["logradouro"] = required("o endereço");
                }
                if (!isset($_POST['selected_simulacao'])) {
                    $retornar["errors"]["option-frete"] = "É necessario escolher uma opção de frete";
                }
            }
        }
        ...
        if ($simulacaoFrete != "") { //valores do frete 
            ...
            if ($calcularCarrinho['data']['status']) {
                ...
                $dtPrevEnt = isset($dadosCarrinho["dtPrevEnt"]) ? formatDateB($dadosCarrinho["dtPrevEnt"]) : '';
                $dtPrevEntMin = isset($dadosCarrinho["dtPrevEntMin"]) ? formatDateB($dadosCarrinho["dtPrevEntMin"]) : '';
                ...
                if ($idSimulacao == "Retirada") {
                    ...
                    $transportadora = utf8_decode($transp_nome . ", Chegará entre $dtPrevEntMin - $dtPrevEnt");
                }
            }
        }
        ...
        /*validação do cupom */
        if ((!empty($cupom) and !empty($msgCupom))) {
            $retornar["errors"]["cupom"] = $msgCupom;
        }
        ...
        if ($codigo_nf == "") { //insert
            ...
            pixel('InitiateCheckout', $dados); //api de conversão
        ...
        if ($operacao and $statusInserirProdutoBd) {
            ...
            $retornar["data"] = array("status" => true, "link_order" => $link_order, "dados_pixel" => $dados_pixel);
        } else {
