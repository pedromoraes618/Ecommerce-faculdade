...
if (isset($_GET['page'])) {
    ...
    $titulo_secao_novidade = utf8_encode(consulta_tabela('tb_parametros', 'cl_id', '113', 'cl_valor'));
    $titulo_secao_desconto = utf8_encode(consulta_tabela('tb_parametros', 'cl_id', '112', 'cl_valor'));
    $titulo_secao_catalogo = utf8_encode(consulta_tabela('tb_parametros', 'cl_id', '115', 'cl_valor'));
    $titulo_secao_marcadores = utf8_encode(consulta_tabela('tb_parametros', 'cl_id', '115', 'cl_valor'));
    ...
    if (!empty($formato)) {
        $descricao_formato = utf8_encode(consulta_tabela('tb_unidade_medida', 'cl_id', $formato, 'cl_descricao')); //descricao do formato/unidade de medida
        $path .= "<li class='breadcrumb-item'><a>$titulo_menu_unidade_medida</a></li><li class='breadcrumb-item'><a>$descricao_formato</a></li>";
    }
    if (!empty($marcador)) {
        $titulo_menu_marcadores = utf8_encode(consulta_tabela('tb_parametros', 'cl_id', 139, 'cl_valor'));
        $descricao_marcador = utf8_encode(consulta_tabela('tb_marcadores', 'cl_id', $marcador, 'cl_descricao')); //descricao do marcador
        $path .= "<li class='breadcrumb-item'><a>$titulo_menu_marcadores</a></li><li class='breadcrumb-item'><a>$descricao_marcador</a></li>";
    }
}
