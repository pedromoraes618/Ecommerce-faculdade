...
if (isset($_POST['form'])) {
        ...
        if (!empty($email) and $email == $validaEmail) {
            ...
            <div style='width:100%;max-width:600px;margin:0 auto;text-align:center;'>
                <div style='padding:10px;text-align:left;font-family:Arial, sans-serif;'>
                   ...
                   <p>Recebemos sua solicitação de mudança de senha na nossa loja. Clique no link abaixo para criar uma nova senha</p>
                    <a href=$url_init/$nome_ecommerce/?forgot-password=true&code=$chave' style='display: inline-block; padding: 10px 20px; font-size: 16px; color: #fff; background-color: #111C2E; text-decoration: none; border-radius: 5px; margin: 10px 0;' target='_blank'>Criar Nova senha</a><br><br>
                    <p>Se o link não funcionar, copie e cole este URL no seu navegador: $url_init/$nome_ecommerce/?forgot-password=true&code=$chave</p>
                    <p>Se você não solicitou essa mudança, por favor, entre em contato conosco.</p>
                </div>
            </div>";
            ...
            if ($sendEmail) {
                $retornar["data"] = array("status" => true, "message" => "Email de confirmação enviado com sucesso", "email" => $email);
    ...
    if ($acao == "resetPassword") {
        ...
        if (empty($token)) {
            ...
            <div>Link não identificado, favor, refaça o processo</div>
          </div>";
        } else {
            ...
            if (!empty($validaToken)) {
                ...
                if ($data > $data_limite) {
                    $retornar["errors"]["senha"] = ("Link expirado, refaça o processo novamente");
                }
            } else {
                $retornar["errors"]["warning"] = "
                <div class='alert alert-danger' role='alert'>
                <div>Email não registrado!</div>
              </div>";
            }
        }
        ...
        if ($reset_senha) {
            $retornar["data"] = array("status" => true, "message" => "Senha redefinida com sucesso!");
        } else {
